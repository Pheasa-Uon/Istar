...
export class UsersComponent {
    ...
    onRoleCheckboxChange(role: RolePermissionModel) {
        ...
        const action$ = role.checked ? this.userService.assignRole(request) : this.userService.removeRole(request);
        ...
        action$.subscribe({
            ...
            error: () => {
                role.checked = !role.checked; // revert change on error
                this.messageService.add({ severity: 'error', summary: 'Error', detail: `Failed to ${role.checked ? 'assign' : 'remove'} role` });
            }
        });
    }
}
